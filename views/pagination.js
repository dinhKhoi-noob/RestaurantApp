...
module.exports = (app) => {
  app.get("/page/index", (req, res) => {
    ...
    fs.readFile(path.resolve(__dirname, fileName), async (err, data) => {
      ...
      try {
        products = await axios.default.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product`
        );
        categories = await axios.default.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/category`
        );
        breakfastDishes = await axios.default.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/service?service='breakfast'`
        );
        lunchDishes = await axios.default.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/service?service='lunch'`
        );
        dinnerDishes = await axios.default.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/service?service='dinner'`
        );
        discountingProducts = await axios.default.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/discount_dish`
        );
      } catch (error) {
        console.log(error);
      }
      return res.render(fileName, {
        data: {
          ...
          discountingProducts: discountingProducts
            ? discountingProducts.data.result
            : null,
  ...
  app.get("/page/category/:id", (req, res) => {
    ...
    fs.readFile(path.resolve(__dirname, fileName), async (err, data) => {
      ...
      try {
        const uri = !root
          ? `http://${process.env.express_host}:${process.env.express_port}/api/product/all/${id}`
          : `http://${process.env.express_host}:${process.env.express_port}/api/product/all/${id}?root=1`;
        const products = await axios.get(uri);
        const categories = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/category`
        );
        const currentCategory = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/category/${id}`
        );
        return res.render(fileName, {
          data: {
    ...
    fs.readFile(path.resolve(__dirname, fileName), async (err, data) => {
      ...
      try {
        const response = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/detail/${id}`
        );
        const dish = response.data.result;
        const categories = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/category`
        );
        const relatedDishes = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/all/${dish[0].category_id}`
        );
        return res.render(fileName, {
          data: {
            dish,
  ...
  app.get("/page/discount_dish/:id", async (req, res) => {
    ...
    fs.readFile(path.resolve(__dirname, fileName), async (err, data) => {
      ...
      try {
        const response = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/discount_dish/${id}`
        );
        ...
        const relatedDishes = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/all/${dish.category_id}`
        );
    ...
    try {
      ...
      try {
        const searchResult = await axios.get(
          `http://${process.env.express_host}:${process.env.express_port}/api/product/all/${categoryId}?root=${isRoot}&search=${searchString}`
        );
