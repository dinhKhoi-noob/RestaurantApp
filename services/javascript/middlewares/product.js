...
const nullCheck = (req, res, next) => {
    const { title, category_id, price } = req.body;
    if (!title || !category_id || !price || title === "" || category_id === "" || price === "" || price === 0)
...
const referenceCheck = (req, res, next) => {
    ...
    try {
        return connection.query(
            `select visible_id,title,category_id from categories where visible_id='${category_id}'`,
            (err, result) => {
                if (result.length < 1) return res.status(404).json({ success: false, message: "Invalid category id" });
                return connection.query(
                    `select visible_id,title,category_id from categories where visible_id='${root_category}'`,
                    (err, result) => {
                        if (result.length < 1) {
