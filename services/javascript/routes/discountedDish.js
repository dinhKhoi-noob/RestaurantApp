...
route.get('/',(req,res)=>{
    try {
        ...
        and CURRENT_TIME BETWEEN d.date_begin AND d.date_end;`,(err,result)=>{
            if(result.length > 0){
...
route.get('/:id',(req,res)=>{
    ...
    try {
        connection.query(`select d.title as 'discount_title',d.desc as 'discount_desc',d.date_begin,d.date_end,d.date_created,
        dd.visible_id,dd.dish_id,dd.discount_id,dd.sale_percent,
        p.category_id,p.root_category,p.thumbnail,p.price,p.service,p.title,p.description 
        from discounted_dishes as DD 
        inner join products as P 
        ...
        and d.date_end and dd.visible_id like '${id}';`,(err,result)=>{
            if(result.length > 0){
...
route.post('/',(req,res)=>{
    ...
    try {
        dishes.forEach(dish=>{
            ...
            connection.query(`insert into discounted_dises(visible_id,dish_id,discount_id,sale_percent) values('${id}','${dish.id}','${discount_id}',${dish.sale_percent})`);
        })
